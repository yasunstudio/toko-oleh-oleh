...
class CSVExporter {
  ...
  async exportCategoriesCSV() {
    ...
    await this.saveCSV('categories', flatCategories)
  ...
  async exportAll() {
    ...
    try {
      ...
      await this.exportCategoriesCSV()
...
async function main() {
  ...
  switch (format) {
    ...
    case 'products':
      ...
      break
    case 'categories':
      await exporter.exportCategoriesCSV()
      break
    case 'orders':
    ...
    default:
      ...
      console.log('  npm run export-csv categories  # Export categories')
